...
contract DeployPeriphery is Script, Artifacts {
    ...
    {
        bytes32 salt = keccak256(abi.encodePacked(bytes(_name), "v1"));
    ...
    {
        // Load the drip config to figure out interval.
        (, Drippie.DripConfig memory dripConfig,,) = _drippie.drips(_name);

        // TRIGGER module allows us to configure time interval.
        GelatoDataTypes.Module[] memory modules = new GelatoDataTypes.Module[](1);
        modules[0] = GelatoDataTypes.Module.TRIGGER;

        // Create arguments for the TRIGGER module.
        bytes[] memory args = new bytes[](1);
        args[0] = abi.encode(
            GelatoDataTypes.TriggerModuleData({
                triggerType: GelatoDataTypes.TriggerType.TIME,
                triggerConfig: abi.encode(GelatoDataTypes.Time({ nextExec: 0, interval: uint128(dripConfig.interval) }))
            })
        );
    ...
    function _installDrip(IGelato _gelato, Drippie _drippie, string memory _name, Drippie.DripConfig memory _config) internal {
